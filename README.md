...
The built React files are copied to:
...
The custom configuration is useful for controlling how Nginx handles requests, particularly for React applications using client-side routing.
...
can be handled through the React application's index.html.

Why Nginx?

Instead of running the React development server in the final container, the production build is served as static files by Nginx.

This provides a cleaner and significantly smaller runtime image.
...
The application was tested directly using:

http://localhost:3500
🚀 Backend Multi-Stage Build

The backend was also optimized using a multi-stage approach.
...
The final image contains only the dependencies and files required to run the backend.
...
A specific MySQL version is used instead of relying on:

latest

Pinning the version helps avoid unexpected behavior when a new MySQL version is released.
...
The three services need to communicate with one another.

With Docker Compose, a network is automatically created for the services.

The backend can communicate with the database using the Compose service name:
...
the entire application can be managed through:
...
The backend connects to the database using the Compose service name.

The backend also waits for the database healthcheck to succeed before starting.
...
The backend URL is supplied as a build argument:
...
This is better than simply checking whether the database container has started.
...
The application's healthcheck has successfully passed.

🔐 Environment Variables and Build Arguments

The React frontend requires the backend API URL.
...
For Create React App-style applications, REACT_APP_* values are incorporated into the frontend bundle during the build.

Therefore, changing the value after the React application has already been built does not automatically change the value embedded in the JavaScript bundle.
...
loads the Student–Teacher application.
...
returns a backend response containing application data.
...
The frontend image dropped from approximately:
...
The backend image dropped from approximately:
...
The frontend container was running, but the browser showed:

Welcome to nginx!
Cause

The host's existing Nginx service was already listening on:
...
were reaching the host Nginx service instead of the Docker container.
...
The conflicting Nginx service was stopped so Docker could bind port 80.
...
This prevented external access to the application inside the container.

The server was changed to listen on:

0.0.0.0:3000

allowing connections through Docker's published port.

🐛 3. Frontend Could Load but Could Not Add Data
Problem

The React application loaded successfully, but adding student/teacher data failed.

Cause

The React application's API URL had not been correctly supplied during the frontend image build.

Solution

The backend URL was passed as a build argument:
...
The database volume contained data created using a different MySQL version.

Using:

mysql:latest

allowed the MySQL version to change unexpectedly.

Solution

The MySQL image was pinned to a specific version:

mysql:8.0.40-oraclelinux9

Pinning image versions makes builds more predictable and avoids unexpected version changes.

🐛 5. Database Persistence Testing

The database container was deleted and recreated using the same named volume.

The purpose was to verify that the database data survives container deletion.
...
During one Compose build, Docker failed while downloading the MySQL image with an:

EOF

error from Docker's image distribution/CDN endpoint.

The failure occurred while the image was almost completely downloaded.

This was identified as an image download/network interruption rather than a Docker Compose configuration problem.

Retrying the image pull/build resolved the issue.
...
Some configurations should be improved before using the application in a real production environment.
...
This is acceptable for this local learning project, but should not be used in a real production environment.
...
The backend currently uses the MySQL root account.

A production application should use a dedicated database user with only the required permissions.
...
Pinning versions improves reproducibility and reduces unexpected breaking changes.

4. Run Containers as Non-Root

The backend runtime was configured to use a non-root user.

Running applications as non-root reduces the impact of a potential container compromise.

5. Production HTTPS

A real deployment should use HTTPS/TLS rather than exposing the application directly over HTTP.

A reverse proxy or load balancer could be introduced for production deployment.
...
A named Docker volume was introduced for MySQL:

mysql-data

This allowed database data to survive container recreation.

Phase 4 — Multi-Stage Builds

Frontend and backend images were optimized using multi-stage Dockerfiles.

The frontend was changed to:
...
All services were moved into a single Compose configuration.
...
Your current Compose file contains:

```yaml
MYSQL_ROOT_PASSWORD: mysql123

Since this is a learning project, that's fine for demonstrating the setup, but I would change the README example to use placeholders such as:

MYSQL_ROOT_PASSWORD: <your-password>

and preferably commit a:

.env.example

rather than a real .env.

The instructor notes explicitly treated the hardcoded password as a local-learning convenience, not a production practice.

2. Don't claim you developed the Student–Teacher application.
The wording in the README above deliberately separates the instructor-provided application from your Docker/DevOps contribution. That's the honest way to present it.

3. Definitely include the image-size comparison.
It's one of the strongest parts of this project:

Frontend: 594 MB → 49 MB
Backend:  891 MB → 152 MB

Those figures are documented in the project notes.

4. Include your debugging experience.
Don't hide the errors you encountered. The port-80 Nginx conflict, frontend API build argument problem, MySQL version issue, and Compose troubleshooting make the project look more practical, not less. The project notes also document the Compose healthcheck/version issue and the successful end-to-end verification.

5. One correction I'd make to your repository before publishing: remove this from the Compose file:

version: '3.9'

Modern Docker Compose doesn't require the top-level version field, and you already saw the warning about it. The project notes also explicitly recommend omitting it.

This README will make the repository look like a proper Docker learning/project repository, rather than simply a collection of Dockerfiles.
